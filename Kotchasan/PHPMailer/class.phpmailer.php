...
class PHPMailer
{
     ...
     * @see https://tools.ietf.org/html/rfc5321#section-4.4 RFC5321 reference
     */
    public $ReturnPath = '';
    /**
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                $result = pclose($mail);
                $this->doCallback(
                    ($result == 0), [$toAddr], $this->cc, $this->bcc, $this->Subject, $body, $this->From
                );
                if ($result != 0) {
                    throw new phpmailerException($this->lang('execute').$this->Sendmail, self::STOP_CRITICAL);
                }
     ...
     *
     * @param array $addr A 2-element indexed array, element containing an address, element 1 containing a name
     *
     * @return string
     */
    ...
    public function createHeader()
    {
        ...
        foreach ($this->CustomHeader as $header) {
            $result .= $this->headerLine(
                trim($header[0]), $this->encodeHeader(trim($header[1]))
            );
        }
            ...
            try {
                ...
                if (empty($this->sign_extracerts_file)) {
                    $sign = @openssl_pkcs7_sign(
                        $file, $signed, 'file://'.realpath($this->sign_cert_file), ['file://'.realpath($this->sign_key_file), $this->sign_key_pass], null
                    );
                } else {
                    $sign = @openssl_pkcs7_sign(
                        $file, $signed, 'file://'.realpath($this->sign_cert_file), ['file://'.realpath($this->sign_key_file), $this->sign_key_pass], null, PKCS7_DETACHED, $this->sign_extracerts_file
                    );
                }
                if ($sign) {
                    @unlink($file);
     ...
     *
     * @see http://www.php.net/manual/en/function.quoted-printable-decode.php#89417 Adapted from this comment
     *
