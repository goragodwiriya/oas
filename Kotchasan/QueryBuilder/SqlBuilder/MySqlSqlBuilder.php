...
class MySqlSqlBuilder extends AbstractSqlBuilder
{
    ...
    protected function buildExistsClause(array $where, array &$bindings): string
    {
        ...
        if (is_array($table)) {
            // First table is FROM, rest are JOINs
            $tables = $table;
            ...
            // Add remaining tables as implicit JOINs (comma-separated)
    ...
    {
        // If it looks like a column reference (contains dot or starts with table alias pattern)
        if (is_string($value) && preg_match('/^[A-Za-z_][A-Za-z0-9_]*\.[A-Za-z_][A-Za-z0-9_]*$/', $value)) {
            // It's a column reference like 'P.id' or 'D.product_id'
            return $this->quoteIdentifier($value);
        }

        // It's a literal value - add to bindings
        $bindings[] = $value;
        return '?';
    }
