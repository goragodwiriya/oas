 ...
 *
 * PostgreSQL-specific SQL builder implementation.
 *
 * @package Kotchasan\QueryBuilder\SqlBuilder
 */
class PostgreSqlSqlBuilder extends AbstractSqlBuilder
{
    /**
     * {@inheritdoc}
     */
    ...
    public function buildInsertStatement(string $table, array $data, array &$bindings, bool $ignore = false): string
    {
        ...
        if (isset($data[0]) && is_array($data[0])) {
            ...
            $sql = 'INSERT INTO '.$quotedTable.' ('.implode(', ', $quotedColumns).') VALUES '.implode(', ', $values);
            if ($ignore) {
