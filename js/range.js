...
(function () {
  ...
  GRange.prototype = {
      ...
      if (!$E(id)) {
        ...
        this.slider.addEvent('mousedown', function (e) {
          ...
          if (elem == HInstant.slider || elem == HInstant.range) {
            var pos = GEvent.pointerX(e) - this.viewportOffset().left - (HInstant.buttonWidth / 2),
        ...
        this.slider.addEvent('click', function (e) {
          var elem = GEvent.element(e);
          if (elem && (elem == HInstant.pointerL || elem == HInstant.pointerR)) {
            elem.focus();
          }
        });
        ...
        this.timer = window.setInterval(function () {
          ...
          } else if (HInstant.oldvalue.value != HInstant.input.value ||
    ...
    setX: function (button, pos) {
      ...
      var val = floatval(this.input.min) + (this.getRange() * pos) / this.getWidth();
      if (this.pointerR) {
    ...
    calcValue: function (value) {
      ...
      value = Math.min(floatval(this.input.max), Math.max(floatval(this.input.min), value));
      if (ss[1] && ss[1] != '') {
