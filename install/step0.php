...
if (defined('ROOT_PATH')) {
    ...
    $check = array(
        ($v ? 'เวอร์ชั่นของ PHP <b>'.PHP_VERSION.'</b>' : 'ต้องการ PHP เวอร์ชั่น <b>5.6.0</b> ขึ้นไป') => $v ? 'correct' : 'incorrect',
    ...
    echo '<li class='.(function_exists('idn_to_ascii') ? 'correct' : 'incorrect').'>Intl support <b>ON</b></li>';
    echo '<li class='.(function_exists('opcache_invalidate') ? 'correct' : 'incorrect').'>OPcache support <b>ON</b></li>';
    echo '</ul>';
    if ($error) {
