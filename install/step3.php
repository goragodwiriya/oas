...
if (defined('ROOT_PATH')) {
    if (empty($_POST['username']) || empty($_POST['password'])) {
        ...
        echo '<p class=item><label for=db_username>ชื่อผู้ใช้</label><span class="form-control icon-user"><input type=text size=50 id=db_username name=db_username value="'.$db_username.'"></span></p>';
        echo '<p class=comment>ชื่อผู้ใช้ของ MySQL ของคุณ</p>';
        echo '<p class=item><label for=db_password>รหัสผ่าน</label><span class="form-control icon-password"><input type=password size=50 id=db_password name=db_password value="'.$db_password.'"></span></p>';
        echo '<p class=comment>รหัสผ่านของ MySQL ของคุณ</p>';
        echo '<p class=item><label for=db_server>โฮสท์ของฐานข้อมูล</label><span class="form-control icon-world"><input type=text size=50 id=db_server name=db_server value="'.$db_server.'"></span></p>';
        echo '<p class=comment>ดาตาเบสเซิร์ฟเวอร์ของคุณ (โฮสท์ส่วนใหญ่ใช้ localhost)</p>';
        echo '<p class=item><label for=db_port>พอร์ต</label><span class="form-control icon-link"><input type=text size=50 id=db_port name=db_port value="'.$db_port.'"></span></p>';
        echo '<p class=comment>ดาตาเบสเซิร์ฟเวอร์พอร์ตของคุณ (โฮสท์ส่วนใหญ่ใช้ 3306)</p>';
        echo '<p class=item><label for=db_name>ชื่อฐานข้อมูล</label><span class="form-control icon-database"><input type=text size=50 id=db_name name=db_name value="'.$db_name.'"></span></p>';
        echo '<p class=comment>ชื่อฐานข้อมูลที่ใช้ในการติดตั้งโปรแกรม ต้องเป็นฐานข้อมูลที่มีอยู่แล้ว หรือสร้างฐานข้อมูลก่อน</p>';
        echo '<p class=item><label for=prefix>คำนำหน้าตาราง</label><span class="form-control icon-table"><input type=text size=50 id=prefix name=prefix value="'.$prefix.'"></span></p>';
        ...
        echo '<p class="submit"><button class="btn large btn-primary" type=submit>ติดตั้ง</button></p>';
        echo '</form>';
    }
}
