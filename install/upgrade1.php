...
if (defined('ROOT_PATH')) {
    ...
    echo '<p class=item><label for=username>ชื่อผู้ใช้</label><span class="form-control icon-user"><input type=text size=50 maxlength=50 id=username name=username value="'.$username.'"></span></p>';
    if (empty($username)) {
    ...
    echo '<p class=item><label for=password>รหัสผ่าน</label><span class="form-control icon-password"><input type=password size=50 maxlength=20 id=password name=password value="'.$password.'"></span></p>';
    if (empty($password)) {
