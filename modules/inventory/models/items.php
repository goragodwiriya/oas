            ...
            if (Login::checkPermission($login, 'can_manage_inventory') && Login::notDemoMode($login)) {
                try {
                    ...
                    if ($index) {
                        ...
                        $db = $this->db();
                        // รับค่าจากการ POST
                        $product_no = $request->post('product_no', array())->topic();
                        $topic = $request->post('topic', array())->topic();
                        $price = $request->post('price', array())->toFloat();
                        $unit = $request->post('unit', array())->topic();
                        $cut_stock = $request->post('cut_stock', array())->toFloat();
                        $items = array();
