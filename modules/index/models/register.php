      ...
      if ($index && ($login['id'] == $index['id'] || $login['status'] == 1)) {
        if ($save['username'] == '') {
          ...
          if (empty($ret)) {
            ...
            if ($index['id'] == 0) {
              // register
              \Index\Register\Model::execute($model, $save, $save['permission']);
              // ไปหน้ารายการสมาชิก
              ...
              // permission
              if (!empty($save['permission']) && ($login['status'] == 1 || ($login['status'] == 2 && $login['id'] != $index['id']))) {
                $save['permission'] = implode(',', $save['permission']);
              } else {
